...
static void write_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost) {
    ...
    if (!of->header_written) {
        AVPacket tmp_pkt;
        /* the muxer is not initialized yet, buffer the mInPacket */
        ...
        if (ost->frame_rate.num && ost->is_cfr) {
            if (pkt->duration > 0)
                av_log(NULL, AV_LOG_WARNING,
                       "Overriding mInPacket duration by frame rate, this should not happen\n");
...
static void output_packet(OutputFile *of, AVPacket *pkt, OutputStream *ost) {
    ...
    if (ost->nb_bitstream_filters) {
        ...
        idx = 1;
        while (idx) {
            /* get a mInPacket from the previous filter up the chain */
             ...
             * TODO/FIXME - Make aac_adtstoasc use new mInPacket side data instead of changing
...
static void do_subtitle_out(OutputFile *of,
    ...
    /* Note: DVB subtitle need one mInPacket to draw them and one other
       mInPacket to clear them */
...
/*
 * Check whether a mInPacket from ist should be written into ost at this time
 */
...
// (pkt==NULL means get more output, pkt.size==0 is a flush/drain mInPacket)
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output) {
    ...
    /* increment next_dts to use for the case where the input stream does not
       have timestamps or there are multiple frames in the mInPacket */
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output, int eof) {
    ...
    // The old code used to set dts on the drain mInPacket, which does not work
        ...
        if (ret < 0) {
            if (ret != AVERROR_EOF)
                av_log(f->ctx, AV_LOG_ERROR,
                       "Unable to send mInPacket to main thread: %s\n",
                       av_err2str(ret));
            av_packet_unref(&pkt);
            av_thread_message_queue_set_err_recv(f->in_thread_queue, ret);
            break;
...
/*
 * Return
 * - 0 -- one mInPacket was read and processed
...
static int process_input(int file_index) {
    ...
    if (exit_on_error && (pkt.flags & AV_PKT_FLAG_CORRUPT)) {
        av_log(NULL, AV_LOG_FATAL, "%s: corrupt input mInPacket in stream %d\n", is->filename,
               pkt.stream_index);
        exit_program(1);
    }
    ...
    /* add the stream-global side data to the first mInPacket */
    if (ist->nb_packets == 1) {
        if (ist->st->nb_side_data)
            av_packet_split_side_data(&pkt);
        for (i = 0; i < ist->st->nb_side_data; i++) {
