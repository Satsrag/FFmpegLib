...
namespace ffmpegthumbnailer {
    ...
    VideoThumbnailer::VideoThumbnailer()
            : m_ThumbnailSize(128), m_SeekPercentage(10), m_OverlayFilmStrip(false),
              m_WorkAroundIssues(false), m_ImageQuality(8), m_MaintainAspectRatio(true),
              m_SmartFrameSelection(false) {
    }

    VideoThumbnailer::VideoThumbnailer(int thumbnailSize, bool workaroundIssues,
                                       bool maintainAspectRatio, int imageQuality,
                                       bool smartFrameSelection)
            : m_ThumbnailSize(thumbnailSize), m_SeekPercentage(10),
              m_WorkAroundIssues(workaroundIssues), m_ImageQuality(imageQuality),
    ...
    void VideoThumbnailer::generateThumbnail(const string &videoFile, ImageWriter &imageWriter,
        ...
        if ((!m_WorkAroundIssues) || (movieDecoder.getCodec() !=
                                      "h264")) //workaround for bug in older ffmpeg (100% cpu usage when seeking in h264 files)
        {
            try {
                int secondToSeekTo = m_SeekTime.empty() ? movieDecoder.getDuration() *
                                                          m_SeekPercentage / 100
    ...
    void
    VideoThumbnailer::generateSmartThumbnail(MovieDecoder &movieDecoder, VideoFrame &videoFrame) {
    ...
    void VideoThumbnailer::generateThumbnail(const string &videoFile, ThumbnailerImageType type,
                                             const string &outputFile,
                                             AVFormatContext *pAvContext) {
        ImageWriter *imageWriter = ImageWriterFactory<const string &>::createImageWriter(type,
                                                                                         outputFile);
    ...
    void VideoThumbnailer::generateThumbnail(const string &videoFile, ThumbnailerImageType type,
                                             vector<uint8_t> &buffer, AVFormatContext *pAvContext) {
        buffer.clear();
        ImageWriter *imageWriter = ImageWriterFactory<vector<uint8_t> &>::createImageWriter(type,
                                                                                            buffer);
        generateThumbnail(videoFile, *imageWriter, pAvContext);
        delete imageWriter;
    }

    void VideoThumbnailer::writeImage(const string &videoFile, ImageWriter &imageWriter,
                                      const VideoFrame &videoFrame, int duration,
                                      vector<uint8_t *> &rowPointers) {
        if ((videoFile != "-") && (videoFile.find("rtsp://") != 0) &&
            (videoFile.find("udp://") != 0) && (videoFile.find("http://") != 0)) {
            ...
            if (stat(videoFile.c_str(), &statInfo) == 0) {
                ...
                cout << "Warn: Failed to stat file " << videoFile << " (" << strerror(errno) << ")"
                     << endl;
            }
    ...
    void
    VideoThumbnailer::generateHistogram(const VideoFrame &videoFrame, Histogram<int> &histogram) {
        for (int i = 0; i < videoFrame.height; ++i) {
